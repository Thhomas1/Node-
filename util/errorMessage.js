module.exports={
    GENERAL:{
        campo_obligatorio:"El campo {PATH} es obligatorio",
        minlength:"El campo {PATH} debe contener al menos {VALUE}",
        maxlength:"El campo {PATH} debe contener no mas de {VALUE}",
        lowercase:"El campo {VALUE} no debe contener mayusculas",
        numero_obligatorio:"El campo debe contener numeros",
        nocopy:"El campo {PATH} no debe tener el mismo nombre"
    },
    USERSADMIN:{
        userExist:"El {VALUE} ya existe",
        passwordIncorrect:"El {PATH} debe contener al menos 1 letra, 1 minuscula, 1 mayuscula"
    }
}